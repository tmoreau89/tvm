...
import vta
import vta.testing
from vta.top.testing import run_vta_conv2d, my_clip
import numpy as np
...
def test_cpu_conv2d():
    def run_cpu_conv2d(env, remote, key, batch_size, wl):
...
def test_vta_conv2d():
    def _run(env, remote):
        ...
        for i in range(0, len(resnet)):
            ...
            print(wl)
            (correct, cost, stats) = run_vta_conv2d(env, remote, wl)
            assert(correct)
            gops = (num_ops / cost.mean) / float(10 ** 9)
            print("\tTime cost = %g sec/op, %g GOPS" % (cost.mean, gops))
